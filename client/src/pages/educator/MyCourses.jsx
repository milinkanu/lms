...
const MyCourses = () => {
  ...
  const fetchEducatorCourses = async () => {
    try {
      ...
      const { data } = await axios.get(`${backendUrl}/api/educator/get-educator-courses`, {
        headers: {
  ...
  return courses ? (
    <div className='h-screen flex items-start justify-between gap-8 md:p-8 md:pb-0 p-4 pt-8 pb-0'>
              ...
              {courses.map((course) => (
                <tr key={course._id} className="border-b border-gray-500/20 dark:border-gray-500/50">
                  ...
                  <td className="px-4 py-3">
                    {(course.coursePrice - course.discount * course.coursePrice / 100) === 0 ? "Free" : `${currency} ${(course.coursePrice - course.discount * course.coursePrice / 100).toFixed(2)}`}
                  </td>
